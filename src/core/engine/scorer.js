...
export function v5Formula(input, gamma = 0.85, barrier = 0.5) {
  ...
  gamma = Math.max(0.1, Math.min(2.0, gamma));

  if (barrier === null || barrier === undefined || isNaN(barrier)) {
    barrier = 0.5;
  }
  barrier = Math.max(0, Math.min(1, barrier));
 ...
 */
export function batchCalculateRecall(
  input,
  memories,
  gamma = 0.85,
  barrier = 0.5,
